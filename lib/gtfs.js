...
var ClusterToLine = db.model('ClusterToLine');
...
module.exports = {
  ...
  getClusterToLine: function(agency_key, line_id, cb) {
    ClusterToLine.findOne({
        agency_key: agency_key,
        office_line_id: line_id
    }, cb);
  },

  getClustersForAgencyId: function (agency_key, agency_id, cb) {
    Agency.findOne({
      agency_key: agency_key,
      agency_id: agency_id
    }).exec(function (error, agency) {
      findClustersWithAgencyName(agency_key, agency.agency_name, cb);
    });
  },

  getClustersForAgencyName: function (agency_key, agency_name, cb) {
    findClustersWithAgencyName(agency_key, agency_name, cb);
  }
};

function findClustersWithAgencyName(agency_key, agency_name, cb) {
  ClusterToLine.aggregate([
    { $match: {
      agency_key: agency_key,
    ...
    { $group: {
      _id: {cluster_name: '$cluster_name', cluster_id: '$cluster_id', cluster_sub_desc: '$cluster_sub_desc'}
      }
    }
  ]).exec(function(e, clusters) {
    var mappedClusters = clusters.map(function (cluster) {
      return {
        cluster_name: cluster._id.cluster_name,
        cluster_id: cluster._id.cluster_id,
        cluster_sub_desc: cluster._id.cluster_sub_desc
      };
    });
